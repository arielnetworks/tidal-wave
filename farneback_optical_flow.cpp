...
//w = cv2.calcOpticalFlowFarneback(img1, img2, 0.5, 3, 30, 3, 7, 1.5, cv2::OPTFLOW_FARNEBACK_GAUSSIAN)
    d_calc.pyrScale = 0.5;
    d_calc.numLevels = 3;
    d_calc.winSize = 30;
    d_calc.numIters = 3;
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        cout << "mode: " << (gpuMode?"GPU":"CPU") << std::endl;

        cout << "opt. flow FPS: " << cvRound((getTickFrequency()/(tc1-tc0))) << std::endl;
        cout << "time: " << (tc1-tc0)/getTickFrequency() << std::endl;

        cout << "total FPS: " << cvRound((getTickFrequency()/(t1-t0))) << std::endl;
